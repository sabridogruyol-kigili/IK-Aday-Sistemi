...
export default async function TaleplerPage() {
  ...
  const { data: talepler, error: talepHata } = await supabase
    .from("talepler")
    .select("id, talep_no, talep_turu, pozisyon_tipi, kisi_sayisi, durum, aktif_gonderim_no, created_at, acan_kullanici_id, magaza_grup_id, ise_alimi_tamamlandi_tarihi, magazalar!magaza_id(magaza_adi, bolgeler(ad)), acan:kullanicilar!acan_kullanici_id(ad_soyad, rol)")
