...
export async function olumsuzReferansaEkle(tcKimlikNo: string, adSoyad: string, aciklama: string): Promise<Sonuc> {
  ...
  const { data: mevcut } = await supabase
    ...
    .maybeSingle();
  if (mevcut) {
    return { error: mevcut.durum === "AKTIF" ? "Bu TC zaten olumsuz referans listesinde." : "Bu TC için zaten onay bekleyen bir kayıt var." };
  }

  const durum = me.rol === "YONETIM" ? "AKTIF" : "ONAY_BEKLIYOR";

  const { error } = await supabase.from("olumsuz_referans_listesi").insert({
    tc_kimlik_no: tc,
  ...
  revalidatePath("/adaylar/olumsuz-referans");
  return {};
}

export async function olumsuzReferansOnayla(id: string): Promise<Sonuc> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: "Giriş yapmalısınız." };
  const { data: me } = await supabase.from("kullanicilar").select("id, rol").eq("email", user.email).single();
...
export async function olumsuzReferansReddet(id: string): Promise<Sonuc> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { error: "Giriş yapmalısınız." };
  const { data: me } = await supabase.from("kullanicilar").select("id, rol").eq("email", user.email).single();
...
export type OlumsuzReferansUyarisi = { aciklama: string; durum: "AKTIF" | "ONAY_BEKLIYOR"; ekleyen_rol: string; created_at: string };

export async function olumsuzReferansSorgula(tcKimlikNo: string): Promise<OlumsuzReferansUyarisi | null> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;
  ...
  return (data as OlumsuzReferansUyarisi) ?? null;
}

export async function getOlumsuzReferansListesi(): Promise<OlumsuzReferansKaydi[]> {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return [];
