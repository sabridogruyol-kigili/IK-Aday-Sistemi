...
import { karaListeyeEkle } from "./actions";

// Hem Kara Liste sayfasından (TC serbest girilir) hem de Personel Detay'dan
// (TC ve Ad Soyad hazır dolu, salt okunur gelir) çağrılan ortak modal.
export default function KaraListeyeEkleModal({
  ...
  function gonder() {
    ...
    startTransition(async () => {
      const sonuc = await karaListeyeEkle(tc, ad, aciklama);
    ...
    <div className="fixed inset-0 bg-navy-3/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-card border border-gray-200 w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm font-semibold text-navy-3">Kara Listeye Ekle</div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Kara listeye ekleme gerekçesi..."
            ...
            >
              {pending ? "Kaydediliyor..." : "Kara Listeye Ekle"}
            </button>
          </div>
        )}
      </div>
