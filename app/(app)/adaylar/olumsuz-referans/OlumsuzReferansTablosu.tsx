...
export default function OlumsuzReferansTablosu({ kayitlar, rol }: { kayitlar: OlumsuzReferansKaydi[]; rol: string }) {
  ...
  function onayla(id: string) {
    ...
    startTransition(async () => {
      const sonuc = await olumsuzReferansOnayla(id);
  ...
  function reddet(id: string) {
    setHata(null);
    startTransition(async () => {
      const sonuc = await olumsuzReferansReddet(id);
      if (sonuc?.error) setHata(sonuc.error);
    });
  }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-card p-4">
      ...
      {ekleModalAcik && <OlumsuzReferansEkleModal onClose={() => setEkleModalAcik(false)} />}
    </div>
  );
}
