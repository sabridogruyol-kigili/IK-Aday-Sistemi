// Aday Evrak Portalı için paylaşılan sabitler — hem aday tarafı hem İK
// tarafı bu dosyadan besleniyor, tek yerden yönetiliyor.

export type BelgeTipi = "kimlik" | "adli" | "ikamet" | "nufus" | "diploma" | "saglik" | "iskur" | "askerlik" | "sertifika";

export type BelgeTanimi = {
  id: BelgeTipi;
  ad: string;
  etiket: string;
  kosul?: (cinsiyet: string | null) => boolean;
  coklu?: boolean;
  istegeBagli?: boolean;
  secenekli?: boolean; // sağlık raporu gibi, iki farklı alma yolu olan belgeler
  nasilAlinir: string[];
};
...
export const BELGE_LISTESI: BelgeTanimi[] = [
  ...
  },
  {
    ...
    nasilAlinir: ["e-Devlet'e giriş yapın.", "\"Adli Sicil Kaydı Sorgulama\" hizmetini açın.", "Belgeyi PDF olarak indirip yükleyin."],
  },
  {
    id: "ikamet", ad: "İkametgah belgesi", etiket: "e-Devlet · ücretsiz",
    nasilAlinir: ["e-Devlet'te \"Yerleşim Yeri Belgesi\" hizmetini açın.", "Belgeyi PDF olarak indirip yükleyin."],
  },
  {
    id: "nufus", ad: "Nüfus kayıt örneği", etiket: "e-Devlet · aile bilgilerini içeren tür",
    nasilAlinir: ["e-Devlet'te \"Nüfus Kayıt Örneği\" hizmetini açın.", "Belge türü olarak aile bilgilerini içeren seçeneği seçin.", "Belgeyi PDF olarak indirip yükleyin."],
  },
  {
    id: "diploma", ad: "Mezuniyet veya öğrenci belgesi", etiket: "e-Devlet ya da okulunuz",
  ...
  },
  {
    ...
    nasilAlinir: ["e-Devlet'te \"Askerlik Durum Belgesi\" hizmetini açın.", "Belgeyi PDF olarak indirip yükleyin."],
  },
  {
    id: "sertifika", ad: "Sertifikalar", etiket: "Varsa · zorunlu değil", istegeBagli: true, coklu: true,
    nasilAlinir: ["Mesleki eğitim, kurs veya yetkinlik sertifikalarınızı yükleyin.", "Birden fazla dosya yükleyebilirsiniz."],
  },
];

export function belgeGorunurMu(belge: BelgeTanimi, cinsiyet: string | null): boolean {
  if (belge.kosul) return belge.kosul(cinsiyet);
  return true;
}

export const RED_NEDENLERI = ["Okunaklı değil", "Eksik sayfa", "Süresi geçmiş", "Yanlış belge", "Evrak hatalı", "Diğer"] as const;

// TC Kimlik No algoritmik doğrulaması (resmi TCKN algoritması).
export function tcKimlikGecerliMi(v: string): boolean {
