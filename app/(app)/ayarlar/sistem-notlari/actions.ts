...
export async function getSistemNotlari(): Promise<SistemNotu[]> {
  ...
  const { data, error } = await supabase
    .from("sistem_notlari")
    .select("id, metin, durum, created_at, updated_at, olusturan:kullanicilar!olusturan_kullanici_id(ad_soyad), guncelleyen:kullanicilar!guncelleyen_kullanici_id(ad_soyad)")
    .order("durum", { ascending: true })
    .order("created_at", { ascending: false });
  if (error || !data) return [];

  return (data as any[]).map((n) => ({
    id: n.id, metin: n.metin, durum: n.durum, created_at: n.created_at, updated_at: n.updated_at,
    olusturan_ad_soyad: n.olusturan?.ad_soyad ?? "—",
    guncelleyen_ad_soyad: n.guncelleyen?.ad_soyad ?? null,
  }));
}

export async function sistemNotuEkle(metin: string): Promise<{ error?: string }> {
  const { supabase, me } = await yetkiKontrol();
  ...
  const { error } = await supabase.from("sistem_notlari").insert({ metin: temiz, olusturan_kullanici_id: me.id });
