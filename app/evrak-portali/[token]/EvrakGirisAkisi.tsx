...
export default function EvrakGirisAkisi({ token }: { token: string }) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center p-4">
      <div className="bg-white border border-gray-200 rounded-card p-5 max-w-sm w-full">
        <div className="text-sm font-semibold text-navy-3 mb-1">Evrak Portalı</div>
        ...
        {adim === "email" && (
          <>
            ...
            <label className="block text-[10px] font-semibold text-navy-3 uppercase mb-1">E-posta Adresiniz</label>
            <input
              type="email"
              value={email}
            ...
            />
            {hata && <div className="text-xs text-danger mb-3">{hata}</div>}
        ...
        {adim === "kod" && (
          <>
            <div className="text-xs text-gray-500 mb-4">
              <strong>{email}</strong> adresine 6 haneli bir kod gönderdik. Aşağıya girip devam edin.
            </div>
            <label className="block text-[10px] font-semibold text-navy-3 uppercase mb-1">Doğrulama Kodu</label>
            <input
              value={kod}
              onChange={(e) => setKod(e.target.value.replace(/\D/g, "").slice(0, 6))}
              placeholder="------"
              inputMode="numeric"
              className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm mb-3 text-center font-mono text-lg tracking-[0.3em]"
            />
            {hata && <div className="text-xs text-danger mb-3">{hata}</div>}
            ...
            <button onClick={() => { setAdim("email"); setKod(""); setHata(null); }} className="w-full text-center text-[11px] text-gray-400 mt-3">
